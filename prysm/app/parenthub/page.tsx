      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="grid lg:grid-cols-2 gap-16 items-center mb-16">
              ...
              <div className="relative bg-[var(--prysm-bg)] rounded-2xl p-6 border border-[var(--border-color)]">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="px-3 py-1 bg-[var(--lime)]/10 rounded-full border border-[var(--lime)]/30">
                    <span className="text-[10px] text-[var(--lime)] font-bold uppercase">Parent View</span>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="bg-[var(--bg-overlay)] rounded-xl p-4 border border-[var(--border-color)]">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-[var(--text-secondary)]">Study Time Today</span>
                      <span className="text-sm font-bold text-[var(--lime)]">3h 24m</span>
                    </div>
                    <div className="h-2 bg-[var(--bg-overlay)] rounded-full overflow-hidden">
                      <div className="h-full bg-[var(--lime)]/30 rounded-full" style={{ width: '68%' }}></div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-[var(--bg-overlay)] rounded-lg p-3 border border-[var(--border-color)]">
                      <p className="text-xs text-[var(--text-secondary)] mb-1">Math</p>
                      <p className="text-lg font-bold text-[var(--text-primary)]">45m</p>
                      <div className="flex items-center gap-1 mt-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-[var(--lime)]"></div>
                        <span className="text-[10px] text-[var(--text-tertiary)]">Active</span>
                      </div>
                    </div>
                    <div className="bg-[var(--bg-overlay)] rounded-lg p-3 border border-[var(--border-color)]">
                      <p className="text-xs text-[var(--text-secondary)] mb-1">Biology</p>
                      <p className="text-lg font-bold text-[var(--text-primary)]">1h 12m</p>
                    </div>
                  </div>
                  <div className="bg-[var(--lime)]/10 rounded-lg p-3 border border-[var(--lime)]/20">
                    <p className="text-xs font-bold text-[var(--lime)] uppercase mb-1">Upcoming Exam</p>
                    <p className="text-sm text-[var(--text-primary)] font-semibold">Biology Paper 2</p>
                    <p className="text-xs text-[var(--text-secondary)] mt-1">Nov 15, 2025</p>
                  </div>
                </div>
              </div>
            </Card>
          ...
          <div className="order-1 lg:order-2 text-center lg:text-left">
            <span className="px-5 py-2 rounded-full text-xs font-bold tracking-widest uppercase mb-6 inline-block bg-[var(--prysm-bg)] text-[var(--text-secondary)] border border-[var(--border-color)] [data-theme='light']:bg-[rgba(224,215,255,0.1)] [data-theme='light']:text-[#e0d7ff] [data-theme='light']:border-[rgba(224,215,255,0.3)]">
              Coming Phase 3
            </span>
